...
public class NeuralNet
{
	...
	private void propagateForward (double[] outputPat, double[] inputPat)
	{

	  int length = layer.length;
         int o = length - 1;

        this.layer[0].setOutput(outputPat);
        for (int i = 0; i < length - 1; ++i) {
            this.conn[i].forward();
        }
        this.layer[o].setDelta(layer[o].getDiscrepancy().delta(inputPat, layer[length - 1].getOutput()));
        this.E += this.layer[o].getDiscrepancy().error(inputPat, layer[length - 1].getOutput());
        this.acc += this.classErr(inputPat, layer[length - 1].getOutput());
	...
	private void propagateBackward (boolean learn)
	{
		...
		for (int i = this.layer.length - 2; i >= 0; --i) {
            this.conn[i].backward(learn);
		}
	...
	private void doEpoch (boolean training) 
	{
		...
		while (data.hasNext ())
		{
			propagateForward(data.getNextInputPattern(), data.getNextOutputPattern());
            this.propagateBackward(training);
            data.advance();
